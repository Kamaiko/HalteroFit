-- Fix: Add soft-delete cascade to push_changes RPC
-- Date: 2026-03-08
--
-- When a parent record is soft-deleted (_status = 'deleted'), children were
-- left as orphans in Supabase. ON DELETE CASCADE only applies to hard deletes.
-- The client already sends individual child deletions, but this server-side
-- cascade protects against edge cases and future code paths.
...
BEGIN
  ...
  -- 5. workout_exercises (+ cascade exercise_sets)
  FOR _id IN SELECT * FROM jsonb_array_elements_text(
    COALESCE(changes->'workout_exercises'->'deleted', '[]'::jsonb)
  )
  LOOP
    -- Cascade: soft-delete orphaned exercise_sets
    UPDATE public.exercise_sets SET _status = 'deleted', _changed = _ts
    WHERE workout_exercise_id = _id::UUID AND _status IS NULL;
  ...
  -- 4. plan_days (+ cascade plan_day_exercises)
  FOR _id IN SELECT * FROM jsonb_array_elements_text(
    COALESCE(changes->'plan_days'->'deleted', '[]'::jsonb)
  )
  LOOP
    -- Cascade: soft-delete orphaned plan_day_exercises
    UPDATE public.plan_day_exercises SET _status = 'deleted', _changed = _ts
    WHERE plan_day_id = _id::UUID AND _status IS NULL;
  ...
  -- 3. workouts (+ cascade workout_exercises → exercise_sets)
  FOR _id IN SELECT * FROM jsonb_array_elements_text(
    COALESCE(changes->'workouts'->'deleted', '[]'::jsonb)
  )
  LOOP
    -- Cascade: soft-delete orphaned exercise_sets (via workout_exercises)
    UPDATE public.exercise_sets SET _status = 'deleted', _changed = _ts
    WHERE workout_exercise_id IN (
      SELECT id FROM public.workout_exercises WHERE workout_id = _id::UUID
    ) AND _status IS NULL;

    -- Cascade: soft-delete orphaned workout_exercises
    UPDATE public.workout_exercises SET _status = 'deleted', _changed = _ts
    WHERE workout_id = _id::UUID AND _status IS NULL;

    UPDATE public.workouts SET _status = 'deleted', _changed = _ts
    WHERE id = _id::UUID AND user_id = _uid;
  END LOOP;

  -- 2. workout_plans (+ cascade plan_days → plan_day_exercises)
  FOR _id IN SELECT * FROM jsonb_array_elements_text(
    COALESCE(changes->'workout_plans'->'deleted', '[]'::jsonb)
  )
  LOOP
    -- Cascade: soft-delete orphaned plan_day_exercises (via plan_days)
    UPDATE public.plan_day_exercises SET _status = 'deleted', _changed = _ts
    WHERE plan_day_id IN (
      SELECT id FROM public.plan_days WHERE plan_id = _id::UUID
    ) AND _status IS NULL;

    -- Cascade: soft-delete orphaned plan_days
    UPDATE public.plan_days SET _status = 'deleted', _changed = _ts
    WHERE plan_id = _id::UUID AND _status IS NULL;
