...
This document lists features deferred from the MVP. These enhancements will be prioritized post-launch based on user feedback and product-market fit validation.
...
- **Custom exercise creation** (user-defined exercises with image upload)
  - Migration path documented in ADR-017 _(local archive only — `docs/archives/` is gitignored)_
...
- **License Decision** (If Going Open Source or Private)
  - Current: No license file exists (all rights reserved by default)
  - If going open source: add MIT License file
  - If commercializing: add explicit proprietary license
