...
- **Remove @nozbe/with-observables override** (Dependency Cleanup)
  - TODO: Remove override in package.json when npm publishes v1.7+ with React 19 support
  - Check: `npm info @nozbe/with-observables peerDependencies`
  - GitHub master already has React 19, just not published to npm yet
  - **Estimated effort:** ~5min (just delete the override)
